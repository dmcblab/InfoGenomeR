		...
		if ($e1[2] ne $e2[2]) { # trans
			...
			if ((($e1[1] & 16) and !($e2[1] & 16)) or (!($e1[1] & 16)) and ($e2[1] & 16)) { # inv
				if(abs($pos2 - $pos1) < 10) { # TODO magic number controling inv size
					...
					if ($pos1 < $pos2) {
						if($e1[5]=~/$m1[M].*?$s1[SH]/){
							push @ret, [ ($e2[2], $pos1, "N", ".", "<INV>", ($e1[4]+$e2[4])/2, "PASS", "PRECISE;CT=3to3;CIPOS=-10,10;CIEND=-10,10;SOMATIC;CONSENSUS=$seq;SVTYPE=INV;CHR2=$e2[2];END=$pos2;SVLEN=".abs($pos2-$pos1+1), "GT", "./.",$e2[0]) ] ;
						}else{
							...
							push @ret, [ ($e2[2], $pos2, "N", ".", "<INV>", ($e1[4]+$e2[4])/2, "PASS", "PRECISE;CT=3to3;CIPOS=-10,10;CIEND=-10,10;SOMATIC;CONSENSUS=$seq;SVTYPE=INV;CHR2=$e2[2];END=$pos1;SVLEN=".abs($pos2-$pos1+1), "GT", "./.",$e2[0]) ];
                                                        ...
                                                        push @ret, [ ($e2[2], $pos2, "N", ".", "<INV>", ($e1[4]+$e2[4])/2, "PASS", "PRECISE;CT=5to5;CIPOS=-10,10;CIEND=-10,10;SOMATIC;CONSENSUS=$seq;SVTYPE=INV;CHR2=$e2[2];END=$pos1;SVLEN=".abs($pos2-$pos1+1), "GT", "./.",$e2[0]) ];
						}
					}
				}
			} else {
