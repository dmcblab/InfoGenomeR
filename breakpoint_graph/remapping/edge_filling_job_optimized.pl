...
my $mean_phy_depth = $ARGV[2];
my $library= $ARGV[3];
my $search_length = $ARGV[4];
my $CIGAR = $ARGV[5];
my $read_length = $ARGV[6];
my $chr_prefix = $ARGV[7];
`perl $library/remapping/edge_filling.pl $fasta $search_length $chr_prefix > edge_filling.fa`;
`bwa index edge_filling.fa`;
#`bwa-0.7.15 mem -t 20 -h 20 edge_filling.fa $working_dir/NPE.fq1 $working_dir/NPE.fq2 | samtools view -bS > edge_filling.bam`;
`bwa mem  -k 33 -c 3 -t 40 -h 20 edge_filling.fa $working_dir/NPE.fq1 $working_dir/NPE.fq2 | samtools view -bS > edge_filling.bam`;
...
#`bwa-0.7.15 mem -t 20 -h 20 $fasta $working_dir/NPE.fq1 $working_dir/NPE.fq2 | samtools view -bS > NPE_mapping_to_ref.bam`;
`bwa mem  -k 33 -c 3 -t 40 -h 20 $fasta $working_dir/NPE.fq1 $working_dir/NPE.fq2 | samtools view -bS > NPE_mapping_to_ref.bam`;
...
open($read_info, '<', 'edge_filling_sorted_paired_primary.sam.info');
...
while(my $line = <$read_info>){
	...
	my $read_out;
        if($node1[1]<$node2[1]){
		open($read_out, '>>', "./edge_filling_reads/$line[0].$sign1[0].$line[2].$sign2[0]");
	}else{
                open($read_out, '>>', "./edge_filling_reads/$line[2].$sign2[0].$line[0].$sign1[0]");
	}
...
for(my $unsatisfied_i = 0; $unsatisfied_i <$#unsatisfied; $unsatisfied_i++){
	...
	#       print "\n${ori1_pre}\t$unsatisfied[$unsatisfied_i][2]\t${ori2_pre}\t$unsatisfied[$unsatisfied_j][2]\n";
		 ...
		 $read_in= "./edge_filling_reads/${ori1_pre}.$ori1.${ori2_pre}.$ori2";
