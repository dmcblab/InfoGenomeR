...
for(my $unsatisfied_i = 0; $unsatisfied_i <$#unsatisfied; $unsatisfied_i++){
	...
	#       print "\n${ori1_pre}\t$unsatisfied[$unsatisfied_i][2]\t${ori2_pre}\t$unsatisfied[$unsatisfied_j][2]\n";
		...
		open($read_out, '>', "./edge_filling_reads/${ori1_pre}.$unsatisfied[$unsatisfied_i][2].${ori2_pre}.$unsatisfied[$unsatisfied_j][2]");
		$check=0;
		while(my $line = <$read_info>){	
			...
			if(@line[0] eq $ori1_pre && @sign1[0] eq $ori1 && @line[2] eq $ori2_pre && @sign2[0] eq $ori2){
				shift @sign1;
				shift @sign2;
				$sign1=join("",@sign1);
                                $sign2=join("",@sign2);
		...
		close $read_info;
		 $read_in= "./edge_filling_reads/${ori1_pre}.$unsatisfied[$unsatisfied_i][2].${ori2_pre}.$unsatisfied[$unsatisfied_j][2]";
		if( $check !=0 && $unsatisfied[$unsatisfied_i][3] eq $unsatisfied[$unsatisfied_j][3]){
			$c=`Rscript edge_filling_max_cal.R $read_in $unsatisfied[$unsatisfied_i][3]`;
			@c = split(/\s+/, $c);
			$c = $c[1];
		}else{
			$c = 0;
