args=commandArgs(T)
t=read.table("SVs.CN_opt.AS_SV.haplotype_phased", stringsAsFactors=F)
ochr=c(1:22, "X");
chr=c();
while(nrow(t)!=0){
	chr=c();
	...
	write.table(t[set,],paste("SVs.CN_opt.AS_SV.haplotype_phased.", paste(sort(chr),collapse="."),sep=""),quote=F, sep="\t",row.names=F, col.names=F);
	system(paste("mkdir ",paste("euler.",paste(sort(chr),collapse="."),sep=""),sep=""));
	system(paste(paste("Rscript ", args[1], "/", "TO_bpgraph_hidden_node_for_connectivity_chromosomes_nohidden_ACN.R ", sep=""), paste(sort(chr),collapse=" ")," ", paste("SVs.CN_opt.AS_SV.haplotype_phased.", paste(sort(chr),collapse="."),sep="")));
