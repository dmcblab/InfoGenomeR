...
//       if(newNode_i==528){
 ...
 //                                       cout<<"destche"<<endl;
			...
			total_multi_wto_cycle += multiplicity;

			edges_add.wtis_dest=wtis[1];
			full_edges_add_list.push_back(edges_add);
		}
		int branch_limit=branch_limit_max;
		if(total_multi > 8){
			branch_limit=1;
		}

	//	cout<<"whu_before"<<endl;
	//	list<int***> tcs = total_combinations_full (full_edges_add_list , wtis_src, full_edges_add_list.front().degree1_com, total_multi);a
                list<int***> tcs = set_cover (full_edges_add_list , wtis_src, full_edges_add_list.front().degree1_com, branch_limit);
		cout<<"wto_cycle"<<total_multi_wto_cycle<<endl;
    ...
    //  	 	 }
			...
			for( int i = 1; i <= wtis_loop_i; i++){
				for( int ii =0; ii <4; ii++){
					tcs_e[0][total_multi  - i ][ii] = wtis_loop[0][i-1][ii];
					tcs_e[1][total_multi - i ][ii] = wtis_loop[1][i-1][ii];
				}
			}


                        for( int i = 0; i < total_multi_wto_cycle; i++){
                                for( int ii =0; ii <4; ii++){
					tcs_e[0][i][ii] = (*tcs_i)[0][i][ii];
                                        tcs_e[1][i][ii] = (*tcs_i)[1][i][ii];
				}
			}
                        ...
                        cout<<"tcs:";
                        for(int i=0; i<total_multi; i++){
                                cout<<tcs_e[0][i][0]<<"\t"<<tcs_e[0][i][1]<<"\t"<<tcs_e[0][i][2]<<"\t"<<tcs_e[0][i][3]<<endl;
                        }
                        cout<<endl;
                        for(int i=0; i<total_multi; i++){
                                cout<<tcs_e[1][i][0]<<"\t"<<tcs_e[1][i][1]<<"\t"<<tcs_e[1][i][2]<<"\t"<<tcs_e[1][i][3]<<endl;
                        }
                        cout<<endl;
               ...
               //         if( !tcs_validity(wtis, *tcs_i,  multiplicity,degree1_com, degree2_com)){
                 ...
                 //               continue;
                   //     }
                       ...
                       AdjListNode* newNode = newAdjListNode(i, l_newNode_i, full_edges_list, tcs_e);
...
//                                cout<<"sum:"<<newNode->SV_paths_magic_number<<endl;
		...
		if(total_multi_wto_cycle !=0){
               for(list<int***>::iterator tcs_i= tcs.begin(); tcs_i!=tcs.end(); tcs_i++){
                       for(int i=0; i<2; i++){
                                for(int j=0; j<total_multi_wto_cycle; j++){
                                        delete[] (*tcs_i)[i][j];
                                }
                                delete[] (*tcs_i)[i];
                        }
